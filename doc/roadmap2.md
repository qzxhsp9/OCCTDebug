...
- [ ] 当前 N75 只记录已执行命令时间线，不提供真正的队列调度、逐任务取消按钮、dry-run/subphase 子任务分组；这些留给 N78+。
...
- [ ] 当前 N76 不生成图片像素 diff、属性结构 diff 或性能趋势 diff artifact；真实算法仍需在 N79+ 以后基于该落盘契约逐步启用。

### 36.63 N77：TaskHistory coordinator 拆分

- [x] 新增 `workbench/TaskHistoryCoordinator.*`，把任务 start/finish 记录、状态映射、Case 相对 working directory 归一化、历史上限裁剪和 `WorkbenchMockData` / `CaseManifest` 同步从 `WorkbenchWindow` 抽出。
- [x] `WorkbenchWindow::recordTaskStarted()` / `recordTaskFinished()` 现在只负责调用 coordinator 并刷新 `TaskHistoryPanel`，命令启动时机和 UI 行为保持不变。
